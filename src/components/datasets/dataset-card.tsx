...
const statusConfig: Record<Dataset["status"], { label: string; color: string }> = {
  "on-demand": { label: "On-demand", color: "border-emerald-500/50 text-emerald-400" },
  "custom-collection": { label: "Custom collection", color: "border-amber-500/50 text-amber-400" },
  pilot: { label: "Pilot program", color: "border-blue-500/50 text-blue-400" },
};
      ...
      <div className="glass-card flex h-full flex-col p-6 transition-all duration-300 hover:scale-[1.02] hover:border-primary/20">
        ...
        <div className="mt-4 flex-1 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="font-mono text-xs tabular-nums text-primary">{dataset.capacity}</span>
          </div>
          ...
          <div className="space-y-2 rounded-lg border border-[#1F2937] bg-[#0B0F19]/50 p-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-primary/80">Specifications</p>
          ...
          {/* Proof of quality */}
          <div className="space-y-2 rounded-lg border border-[#1F2937] bg-[#0B0F19]/50 p-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-primary/80">Quality standards</p>
            <div className="grid grid-cols-2 gap-2">
              <div className="flex items-center gap-1.5 text-xs text-[#D1D5DB]">
                <CheckCircle2 className="size-3 text-emerald-400" />
                IAA target: {dataset.qualityDetails.iaa}
              </div>
              <div className="flex items-center gap-1.5 text-xs text-[#D1D5DB]">
              ...
              <div className="flex items-center gap-1.5 text-xs text-[#D1D5DB]">
                {dataset.qualityDetails.biasChecked ? (
                  ...
                  <><FlaskConical className="size-3 text-[#9CA3AF]" />Bias check available</>
                )}
              </div>
            </div>
          </div>
          ...
          {dataset.annotationTypes.length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-primary/80">Annotations included</p>
